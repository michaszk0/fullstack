...
class AddressAutofillInput extends React.Component<AddressAutofillInputProps> {
  ...
  onKeyPress = (e: any) => {
    ...
    if (predictions.length && inputFocused && address) {
      if (key === 'ArrowDown') {
        const value = selectedItem === predictions.length - 1 || selectedItem === null ? 0 : selectedItem + 1;
        this.setState({ selectedItem: value });
      } else if (key === 'ArrowUp') {
        const value = selectedItem === 0 || selectedItem === null ? predictions.length - 1 : selectedItem - 1;
        this.setState({ selectedItem: value });
  ...
  onChangeText = (text: string) => {
    ...
    this.autocomplete.getQueryPredictions({ input: text }, (predictions: MapsPrediction, status: string) => {
      if (status === 'OK') {
