...
const Dot = styled(View)<{ active: boolean }>`
  ...
  height: 6px;
  border-radius: 100px;
  background-color: ${({ active }) => (active ? 'var(--dark-blue)' : 'var(--dark-moderate-blue-30)')};
`;
  ...
  return (
    <Box style={{ height: '75%' }}>
      <BoxInner>
        ...
        />
        <View style={{ position: 'relative', marginBottom: 40, marginTop: 52, alignItems: 'center' }}>
          <DotRow>
            {[0, 1, 2].map((_, i) => (
              <Dot active={i === page} />
            ))}
          </DotRow>
          <Button onPress={page !== 2 ? () => setPage(page + 1) : () => {}} disabled={isNextDisabled[page]}>
            {page === 2 ? 'Done' : 'Next'}
          </Button>
        </View>
      </BoxInner>
