...
import { PLAID_PUBLIC_KEY, PLAID_PRODUCTS, PLAID_COUNTRY_CODES, PLAID_ENV } from '../../../utils/env';
...
class PlaidLink extends React.Component<PlaidLinkProps> {
  ...
  render() {
    return (
      <View>
        <Button disabled={!this.plaidHandler} variant={ButtonVariants.primary} onPress={() => this.plaidHandler.open()}>
          <Image source={{ uri: plaidLogo }} style={{ width: 130, height: 130 }} />
        </Button>
        <Script url="https://cdn.plaid.com/link/v2/stable/link-initialize.js" onLoad={this.onLoadScript} />
      </View>
    );
  }
}
